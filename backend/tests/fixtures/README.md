...
These are used to create a cloned voice profile for every cloning-capable engine (qwen, luxtts). Keep them out of version control if they contain personal audio — this directory is not gitignored by default, so add them to `.gitignore` locally if needed.
